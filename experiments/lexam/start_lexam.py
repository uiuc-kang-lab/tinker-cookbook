...
class Config:
    ...
    num_substeps: int = 1
    wandb_project: str = "tinker-lexam"
    wandb_name: str = "test"
...
class LExamEnv(ProblemEnv):
    ...
    def check_answer(self, sample_str):
        answer_reges = r'(?<!#)###([-+]?\d+)###(?!#)'
        pattern = re.compile(answer_reges)
        matches = [m.group(1) for m in pattern.finditer(sample_str)]
        if len(matches) == 0:
            # print("Env: Action does not contain any answer")
            return False
        if matches[-1] == self.answer:
            # print("Env: Action is correct")
            return True
        else:
            # print("Env: Action is incorrect")
            return False

    async def step(self, action: Action) -> StepResult:
        message, parse_success = self.renderer.parse_response(action)
        # print(f'Action: {message["content"]}')
...
class LExamDataset(RLDataset):
    ...
    def _make_env_group_builder(
        ...
        return ProblemGroupBuilder(
            env_thunk=partial(
                LExamEnv, problem, answer, self.renderer, convo_prefix=self.convo_prefix
            ),
            num_envs=group_size,
...
class ArcDatasetBuilder(RLDatasetBuilder):
    ...
    async def __call__(self) -> tuple[LExamDataset, LExamDataset]:
        ...
        train_dataset_name = "lexam_train.parquet" if self.noise_rate == 0 else f"lexam_train_with_wrong_answers_{self.noise_rate:.1f}.parquet"
        print(f"Loading train dataset from {self.data_path}/{train_dataset_name}")
        test_dataset_name = "lexam_eval.parquet"
        train_dataset = LExamDataset(
